...
import { getShouHuoListApi, deleteCaiGouApl, caigouPayApi, shenpiApi } from "../../apis/userApi";
...
export default {
  ...
  methods: {
    async getList() {
      ...
      let result = await getShouHuoListApi({
        page: this.page,
        size: this.size,
      ...
      switch (type) {
        case "delete":
          this.$dialog
            ...
            .then(async () => {
              let result = await deleteCaiGouApl(item.id);
              if (result.code === 200) {
