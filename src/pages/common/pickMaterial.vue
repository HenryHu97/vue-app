<template>
   <!-- 选择材料 -->
    ...
    <div class="item" @click="scrollEvt">
      <ul>
        <li v-for="(item, index) in list" :key="index" @click="materialConfirm(item)">
...
import { getCityList, getProjectListApi, getMaterialList } from "../../apis/userApi"
...
export default {
  ...
  methods:{
    materialConfirm (user) {
      this.$emit('materialConfirm', user)
